...
func opHash(e *ExecutionEngine) (VMState,error) {
	if e.evaluationStack.Count() < 1 {return FAULT,nil}
...
func opCheckSig(e *ExecutionEngine) (VMState,error) {
	if e.evaluationStack.Count() < 2 {return FAULT,nil}
...
func opCheckMultiSig(e *ExecutionEngine) (VMState,error) {
	if e.evaluationStack.Count() < 4 {return FAULT,nil}
	n :=  int(AssertStackItem(e.evaluationStack.Pop()).GetBigInteger().Int64())
	if n < 1 {return FAULT,nil}
	if e.evaluationStack.Count() < n+2 {return FAULT,nil}
	e.opCount += n
	if e.opCount > e.maxSteps {return FAULT,nil}

	pubkeys := make([][] byte,n)
	for i := 0; i < n; i++ {pubkeys[i] = AssertStackItem(e.evaluationStack.Pop()).GetByteArray()}

	m := int(AssertStackItem(e.evaluationStack.Pop()).GetBigInteger().Int64())
	if m < 1 || m > n {return FAULT,nil}
	if e.evaluationStack.Count() < m {return FAULT,nil}

	signatures := make([][] byte,m)
	for i := 0; i < n; i++ {signatures[i] = AssertStackItem(e.evaluationStack.Pop()).GetByteArray()}
