...
func (self *LevelDBStore) NewDBCache(prefix []byte) IDBCache {
	if self.batch == nil {
		log.Error("[NewDBCache] must call after NewBatch().")
		return nil
	}
	dbcc := NewLevelDBCache(self.db, self.batch, DataEntryPrefix(prefix[0]))
	return dbcc
}