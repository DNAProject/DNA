...
func NewLedgerStore() ILedgerStore {
	...
	cs := NewChainStore("Chain")

	return cs
}

func NewChainStore(file string) *ChainStore {
...
func (bd *ChainStore) persist(b *Block) error {
	...
	// batch write begin
	//bd.st.NewBatch()
	//contracts := bd.st.NewDBCache(ST_Contract)
	//storages := bd.st.NewDBCache(ST_Storage)

	//////////////////////////////////////////////////////////////
	// generate key with DATA_Block prefix
	...
	for i := 0; i < nLen; i++ {
		...
		if b.Transactions[i].TxType == tx.DeployCode {
			...
			if deployCode.InitCode != nil {
				...
				log.Fatal("Contract Execute Result:", se.GetEvaluationStack().Pop())
			}
...
func (bd *ChainStore) onAddHeader(header *Header) {
...
func (db *ChainStore) BlockInCache(hash Uint256) bool {
	db.mu.RLock()
	defer db.mu.RUnlock()
